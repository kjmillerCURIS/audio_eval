                ...
                try:
                    if "gpt" in model.lower():
                        if not self.openai_client:
                            raise ValueError(
                                "OpenAI client not initialized. Please provide an API"
                                " key."
                            )
                        ...
                        try:
                            if not self.gemini_available:
                                raise ValueError(
                                    "Gemini not available. Please provide a Google API"
                                    " key."
                                )
        ...
        if 2 + 2 == 4:
            ...
            if instruction_path and not os.path.exists(instruction_path):
                raise FileNotFoundError(
                    f"Instruction audio file not found: {instruction_path}"
                )

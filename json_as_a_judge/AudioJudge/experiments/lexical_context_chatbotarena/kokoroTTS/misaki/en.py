...
class MutableToken:
    ...
    def stress_weight(self):
        return (
            sum(2 if c in DIPHTHONGS else 1 for c in self.phonemes)
            if self.phonemes
            else 0
        )
...
class Lexicon:
    ...
    def stem_s(self, word, tag, stress, ctx):
        if (
            ...
            and self.is_known(word[:-1], tag)
        ):
            stem = word[:-1]
        elif (
            word.endswith("'s") or (len(word) > 4 and word.endswith("es"))
    ...
    def stem_ing(self, word, tag, stress, ctx):
        if word.endswith("ing") and self.is_known(word[:-3], tag):
            ...
            stem = word[:-3] + "e"
        elif re.search(r"([bcdgklmnprstvxz])\1ing$|cking$", word) and self.is_known(
            word[:-4], tag
        ):
            stem = word[:-4]
        else:
        ...
        if not is_head and "." not in word:
            ...
            if word.isdigit():
                word = num2words(
                    ...
                    else ("year" if not result and len(word) == 4 else "cardinal"),
                )
            elif "." not in word:
                word = num2words(
                    int(word.replace(",", "")),
    ...
    def __call__(self, t, ctx):
        word = (
            (t.text if t.alias is None else t.alias)
            .replace(chr(8216), "'")
            .replace(chr(8217), "'")
        )
...
class G2P:
    ...
    def tokenize(self, text, tokens, features):
        ...
        mutable_tokens = [
            MutableToken(text=t.text, tag=t.tag_, whitespace=t.whitespace_) for t in doc
        ]
        if not features:
            return mutable_tokens
        align = spacy.training.Alignment.from_strings(
            tokens, [t.text for t in mutable_tokens]
        )
    ...
    def retokenize(cls, tokens):
        ...
        for i, token in enumerate(tokens):
            if token.alias is None and token.phonemes is None:
                ts = [
                    replace(token, text=t, whitespace="")
            ...
            for j, t in enumerate(ts):
                if t.alias is not None or t.phonemes is not None:
                    ...
                    t.phonemes = PUNCT_TAG_PHONEMES.get(
                        t.tag, "".join(c for c in t.text if c in PUNCTS)
                    )
                    t.rating = 4
                    # if not t.phonemes:
                    ...
                    0 < j < len(ts) - 1
                    and t.text == "2"
                    and (ts[j - 1].text[-1] + ts[j + 1].text[0]).isalpha()
                ):
                    t.alias = "to"
                if t.alias is not None or t.phonemes is not None:
                    words.append(t)
                    ...
                    words
                    and isinstance(words[-1], list)
                    and not words[-1][-1].whitespace
                ):
                    t.is_head = False
                    words[-1].append(t)
                else:
    ...
    def merge_tokens(cls, tokens, force=False):
        if not force and any(
            t.alias is not None or t.phonemes is not None for t in tokens
        ):
            return None
        text = "".join(t.text + t.whitespace for t in tokens[:-1]) + tokens[-1].text
        tag = max(
            tokens, key=lambda t: sum(1 if c == c.lower() else 2 for c in t.text)
        ).tag
    ...
    def resolve_tokens(cls, tokens):
        ...
        if prespace:
            return
        indices = [
            (PRIMARY_STRESS in t.phonemes, t.stress_weight(), i)
            for i, t in enumerate(tokens)
            if t.phonemes
        ]
        if len(indices) == 2 and len(tokens[indices[0][2]].text) == 1:
            ...
            len(indices) < 2 or sum(b for b, _, _ in indices) <= (len(indices) + 1) // 2
        ):
            return
        ...
        for w in tokens:
            for t in w if isinstance(w, list) else [w]:
                if t.prespace and result and not result[-1].isspace() and t.phonemes:
                    result += " "
                result += (
                    self.unk if t.phonemes is None else t.phonemes
                ) + t.whitespace

...
CONSONANTS = tuple(
    list(CT_convention.C)[:-2]
)  # from the C column of the IPA table, remove special characters # and $
...
def transcribe(
    jamos: str, convention: ConversionTable = CT_convention, str_return: bool = False
) -> Union[list, str]:
    transcribed = []
    for jamo in jamos:
    ...
    for i, syllable in enumerate(syllables_in_jamo):
        try:
            ...
            if next_syllable[0] == "ㅣ":
                ...
                syllables_in_jamo[i] = "".join(
                    list(syllables_in_jamo[i])[:-1] + [new_coda]
                )
        except IndexError:
            continue
...
def simplify_coda(input_word: Word, word_final: bool = False) -> Word:
    ...
    while True:
        double_coda_loc = get_substring_ind(
            input_word.cv, "VCCC"
        )  # get all CCC location
...
def inter_v(symbols: list) -> list:
    voicing_table = {"p": "b", "t": "d", "k": "ɡ", "tɕ": "dʑ"}

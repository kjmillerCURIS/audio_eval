...
#key, text, model_a, model_b, [swap], prediction_content, prediction_vq, prediction_if, prediction_overall, gt_overall_orig, gt_content, gt_vq, gt_if, gt_overall_hcot, gt_overall_blind, dimensionwise_reasoning, fusion_reasoning
...
def make_row(k, results_dict, annotation_dict, is_audiojudge, swap=None):
    ...
    for criteria in CRITERIA:
        if criteria == 'overall (not looking at dimensions)':
            continue
        ...
        row['prediction_' + COLUMN_CRITERIA_MAP[criteria].replace('overall_hcot', 'overall')] = pred

    if is_audiojudge:
        row['dimensionwise_reasoning'] = output['response' + swap]['response']['reasoning']
        row['fusion_reasoning'] = output['fusion_part']['response' + swap]['pred']['reasoning']
    else:
        row['dimensionwise_reasoning'] = output['pred']['reasoning']
        row['fusion_reasoning'] = output['fusion_part']['pred']['reasoning']
...
def make_s2sarena_spreadsheets_one_judge(results_dict, annotation_dict, is_audiojudge, csv_filename):
    rows = []
    if is_audiojudge:
        column_names = ['key', 'text', 'model_a', 'model_b', 'swap', 'prediction_content', 'prediction_vq', 'prediction_if', 'prediction_overall', 'gt_overall_orig', 'gt_content', 'gt_vq', 'gt_if', 'gt_overall_hcot', 'gt_overall_blind', 'dimensionwise_reasoning', 'fusion_reasoning']
    else:
        column_names = ['key', 'text', 'model_a', 'model_b', 'prediction_content', 'prediction_vq', 'prediction_if', 'prediction_overall', 'gt_overall_orig', 'gt_content', 'gt_vq', 'gt_if', 'gt_overall_hcot', 'gt_overall_blind', 'dimensionwise_reasoning', 'fusion_reasoning']
...
def make_s2sarena_spreadsheets():
    ...
    csv_dir = 'json_as_a_judge/s2sarena_experiments/s2sarena_spreadsheets_full_english'
    os.makedirs(csv_dir, exist_ok=True)
    make_s2sarena_spreadsheets_one_judge(AudioJudge_results_dict, annotation_dict, True, os.path.join(csv_dir, 's2sarena_AudioJudge_full_english.csv'))
    make_s2sarena_spreadsheets_one_judge(LLM_judge_results_dict, annotation_dict, False, os.path.join(csv_dir, 's2sarena_LLM_judge_full_english.csv'))
    make_s2sarena_spreadsheets_one_judge(JSON_judge_results_dict, annotation_dict, False, os.path.join(csv_dir, 's2sarena_JSON_judge_full_english.csv'))


if __name__ == '__main__':
    make_s2sarena_spreadsheets()

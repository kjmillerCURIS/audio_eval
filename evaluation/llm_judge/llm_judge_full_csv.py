...
PROMPT_TEMPLATE_PATH = "/projectnb/ivc-ml/ac25/Audio_Eval/audio_eval/evaluation/llm_judge/llm_judge_prompt_multi_aspect.txt"
OUTPUT_CSV_PATH = "/projectnb/ivc-ml/ac25/Audio_Eval/audio_eval/evaluation/llm_judge/output_speakbench_gemini_multi_aspect.csv"
# =====================================
...
def format_prompt(prompt_path, input_prompt, model_a_transcript, model_b_transcript):
    with open(prompt_path, "r") as f:
        template = Template(f.read())

    filled_prompt = template.substitute(
        user_prompt=input_prompt,
        model_a=model_a_transcript,
        model_b=model_b_transcript
    )
...
def main():
    ...
    for entry in all_entries:
        try:
            ...
            audio1_transcript = audio1_json["agent_response"]
            audio2_transcript = audio2_json["agent_response"]

            # --- First prompt (audio1 vs audio2) ---
            prompt1 = format_prompt(PROMPT_TEMPLATE_PATH, instruction_text, audio1_transcript, audio2_transcript)
            print(prompt1)
            ...
            # prompt2 = format_prompt(PROMPT_TEMPLATE_PATH, instruction_text, audio2_transcript, audio1_transcript)
            # print(prompt2)
            # llm_response2 = judge(prompt2, is_json=True)
            # print(llm_response2)
            # prediction_pos_2 = llm_response2.get("label", "").strip()
            # reasoning_pos_2 = llm_response2.get("reasoning", "").strip()
